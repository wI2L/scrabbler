...
func newBag(d distribution) *splitTiles {
	...
	for k, v := range d {
		...
		t := tile{L: unicode.ToUpper(k), letterProps: v}
		if t.kind() == kindVowel {
...
func (g *game) playWord(word string, check bool) error {
	...
	for _, r := range word {
		if idx := cpy.findTile(unicode.ToUpper(r)); idx != -1 {
			// Pop and drop.
			_ = cpy.pickAt(idx)
		} else {
...
func (g *game) drawTiles(rv int) error {
	g.resetDraw(false)
	g.drawCount++

	// Pick the required quantity of vowels,
	// minus any leftover frm previous draw.
	v, err := g.bag.draw(kindVowel, uint(rv-len(g.draw.vowels)), nil)
	if err != nil {
		return err
	}
	g.draw.vowels.add(v...)
	...
	neededConsonants := wordLength - len(g.draw.vowels) - len(g.draw.consonants)
	c, err := g.bag.draw(kindConsonant, uint(neededConsonants), nil)
	if err != nil {
		return err
	}
